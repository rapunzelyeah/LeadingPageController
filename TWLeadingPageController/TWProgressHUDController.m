...
@interface TWProgressHUDController ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation TWProgressHUDController
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch (indexPath.row) {
        case 0:
            [TWProgressHUD showStatus:@"正在努力加载中..."];
            
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(3.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                // 加载完后 移除提示框
                [TWProgressHUD hideHUD];
            });
            break;
        case 1:
            [TWProgressHUD showStatus:@"正在努力加载中..." andAutoHideAfterTime:1.0];
            break;
        case 2:
            
            [TWProgressHUD showSuccess];
            ...
            break;
        case 3:
            [TWProgressHUD showSuccessAndAutoHideAfterTime:1.f];
            break;
        case 4:
            [TWProgressHUD showSuccessWithStatus:@"加载成功!!"];
        ...
        case 5:
            ...
            [TWProgressHUD showSuccessWithStatus:@"加载成功!!" andAutoHideAfterTime:1.f];
            
            break;
        case 6:
            [TWProgressHUD showProgress];
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(3.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                // 加载完后 移除提示框
                [TWProgressHUD hideHUD];
            });
        ...
        case 7:
            ...
            [TWProgressHUD showProgressWithStatus:@"正在努力加载中..."];
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(3.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                // 加载完后 移除提示框
                [TWProgressHUD hideHUD];
            });
            break;
        case 8: {
            /// 内部的HUDView是遵守TWPrivateHUDProtocol的, 在显示的时候会被调整为居中显示
            TWTextOnlyHUDView *custom = [[TWTextOnlyHUDView alloc] init];
            
            custom.text = @"这是自定义的";
            custom.textColor = [UIColor blueColor];
            
            [TWProgressHUD showCustomHUD:custom andAutoHideAfterTime:1.0];
            
        }
            break;
        case 9: {
            // 不遵守TWPrivateHUDProtocol的, 在显示的时候会使用这里设置的frame
...
- (void)original {
    /// 初始化contentView
    TWProgressHUD *contentView = [[TWProgressHUD alloc] initWithFrame:CGRectMake(0.0f, 0.0f, 100.0f, 100.0f)];
